...
def enhancedFeatureExtractorDigit(datum):
    ...
    # Compute gradients to detect the edge
    # ratio of the on pixels
    ...
    for x in range(1, DIGIT_DATUM_WIDTH):
        for y in range(1, DIGIT_DATUM_HEIGHT):
            ...
            if datum.getPixel(x, y)==1: nonzero += 1

    feature['ratio'] = nonzero/(DIGIT_DATUM_WIDTH*DIGIT_DATUM_HEIGHT)
    ...
    for x in xrange(DIGIT_DATUM_WIDTH):
        for y in xrange(DIGIT_DATUM_HEIGHT):
            if (x, y) not in region:
                lookforpixel = datum.getPixel(x, y)
                contiguous += 1
                stack = [(x, y)]
                while stack:
                    point = stack.pop()
                    ...
                    for neighbor in getNeighbors(*point):
                        if datum.getPixel(*neighbor) == lookforpixel and neighbor not in region:
                            stack.append(neighbor)

    features["contiguous"] = contiguous 
    return features
...
def enhancedPacmanFeatures(state, action):
    ...
    if not nearest_food: features["nearest_food"] = 0
    else: features["nearest_food"] = nearest_food[0] * 1.0
